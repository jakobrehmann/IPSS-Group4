...
function initializeNetwork(;
    ...
    elseif network_structure == "regular"
        net = Graphs.random_regular_graph(n_nodes,10)
...
function initialize(net;
    ...
    properties = Dict(
        ...
        # :local_global_weight => local_global_weight,
    ...
    if hom_het == "homogenous"
        for i in 1:n_nodes
            p = Person_Sim(i, 1, base_susceptibility, 0, 0, 0, 1.0, NaN64)
    ...
    elseif hom_het == "heterogenous"
        ...
        for i in 1:(n_nodes)*frac_fearful
            p = Person_Sim(i, 1, base_susceptibility, 0, 0, 1, 0.5,NaN64)
            add_agent_single!(p, model)
        end
        #Now: To the crazy people, second to last argument : 2 → crazy
        for i in ((n_nodes)*frac_fearful+1):(n_nodes)
            p = Person_Sim(i, 1, base_susceptibility, 0, 0, 2, 1.5,NaN64)
            add_agent_single!(p, model)
        end
    elseif hom_het == "heterogenous_assortative"
        ...
        first_agent = Person_Sim(1, 1, base_susceptibility, 0, 0, new_group, new_group_factor,NaN64)
        ...
        while (no_remaining > 0)
            ...
            while length(neighbors) == 0 && no_remaining > 0
                ...
                first_agent = Person_Sim(agent_counter, pos , base_susceptibility, 0, 0, new_group, new_group_factor, NaN64)
            ...
            for neighbor_pos in neighbors
                p = Person_Sim(agent_counter, neighbor_pos, base_susceptibility, 0, 0, new_group, new_group_factor, NaN64)
        ...
        for agent_pos in collect(empty_positions(model))
            p = Person_Sim(agent_counter, agent_pos, base_susceptibility, 0, 0, original_group,original_group_factor, NaN64)
...
function agent_step!(person, model)
    ...
    if person.health_status == 0
        ...
        if(cnt_inf_neighbors > 0)
            calc_infection_chance(model, person, cnt_inf_neighbors, cnt_tot_neighbors)
        end
...
function calc_infection_chance(model, person, cnt_inf_neighbors, cnt_tot_neighbors)
    ...
    n1 = model.prop_agents_infected # global
    n2 = cnt_inf_neighbors / cnt_tot_neighbors # local
    

    # if local & global weight = 0, then reduction factor will be 0, giving us the base-case

    if model.local_weight == "base" || model.global_weight == "base"
        red_factor = 1
    else
        red_factor = model.global_weight * (1 - n1) + model.local_weight * (1 - n2)
    end
